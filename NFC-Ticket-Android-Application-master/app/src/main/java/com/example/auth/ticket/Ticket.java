...
//        }
        ...
        if(checkExpiradateEqualZero(expiryDateFromCard)){
           extendNoR();
           infoToShow = "You haven't use the card for the first time. Add 4 more time to play ";
           return true;
        }
...
//      int month = (int)expiryDate[1] - 1;
      int day = (int)expiryDate[0];
      int hour = (int) expiryDate[1];
      int minute = (int) expiryDate[2];
      int second = (int) expiryDate[3];
      Calendar nowCal = Calendar.getInstance();
      nowCal.clear();
      nowCal.set(Calendar.DAY_OF_MONTH, day);
      nowCal.set(Calendar.HOUR_OF_DAY, hour);
      nowCal.set(Calendar.MINUTE, minute);
      nowCal.set(Calendar.SECOND, second);
...
//        byte[] dateByte = {(byte)(year - 2000), (byte)(month + 1), (byte)day, (byte)0};
        byte[] dateByte = {(byte)day, (byte)hour, (byte)minute, (byte)second};
    ...
    private  boolean checkExpiryDate(byte[] dateInCard){
        ...
        int second = cal.get(Calendar.SECOND);
//        byte[] dateByte = {(byte)(year - 2000), (byte)(month + 1), (byte)day, (byte)0};
        byte[] now = {(byte)day, (byte)hour, (byte)minute, (byte)second};
